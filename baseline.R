############################################################
## LVR_baseline_标准版_修复. R – 修复特殊字符问题
############################################################
...
## ★★★ 关键修复:使用 list() 包装标签,并添加到 modify_header ★★★
tbl <- dat_work %>%
  ...
  ## ★★★ 使用 modify_spanning_header 添加中文标签 ★★★
...
## ★★★ 手动替换变量标签(因为某些特殊字符无法直接传递) ★★★
## 方法1:使用 modify_table_body 直接修改表格内容
...
output_file <- file.path(output_dir, "Table1_Baseline_Fixed.docx")
